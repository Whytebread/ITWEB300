...
$(document).ready(function () {
    ...
    //extracts the ticket number submitted by the user and passes it into the getTicket function
    $("#searchSubmitButton").click(function () {
        const ticketNum = $("#ticketSearch").val();
        getTicket(ticketNum);
    });

    //sends a get request using the ticket number, returns the json data and displays in within a div or provides an error message
    function getTicket(ticketNum) {
        $.ajax({
            type: "GET",
            url: `https://jscript.rdm/ticketrequest.asp?ticket=${ticketNum}`,
            dataType: "json",
            success: function (data) {
                $("#ticketDisplay").html(`
                     <p>Ticket number: ${data.ticketNum}</p>
                     ...
                     <p>Status of the ticket: ${data.ticketStatus}</p>
                     <p>Response provided: ${data.response}</p>   
                    `)
            },
